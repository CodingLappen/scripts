...
AgeingMonitor::sweep_loop() {
  ...
  while (!stop_sweep_thread) {
    ...
    tp += interval;
    stop_condvar.wait_until(lock, tp); // If done earlier it'll wait that long
  }
}

void
AgeingMonitor::do_sweep() {
  for (auto &entry : tables_with_ageing) { //iterate through all sets 
    TableData &data = entry.second;
    const MatchTableAbstract *t = data.table;
    auto &prev_sweep_entries = data.prev_sweep_entries;
    t->sweep_entries(&entries_tmp); //Get entries
    if (entries_tmp.empty()) {
      prev_sweep_entries.clear(); //No Data to be inserted, skip the rest
      continue;
    }
    ...
    for (entry_handle_t handle : entries) {  //Insert the handles in to the list/map prev_sweep_entries
